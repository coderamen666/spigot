...
public class EntityDrowned extends EntityZombie implements IRangedEntity {
    ...
    public static boolean checkDrownedSpawnRules(EntityTypes<EntityDrowned> entitytypes, WorldAccess worldaccess, EnumMobSpawn enummobspawn, BlockPosition blockposition, Random random) {
        if (!worldaccess.getFluidState(blockposition.below()).is(TagsFluid.WATER)) {
            ...
            boolean flag = worldaccess.getDifficulty() != EnumDifficulty.PEACEFUL && isDarkEnoughToSpawn(worldaccess, blockposition, random) && (enummobspawn == EnumMobSpawn.SPAWNER || worldaccess.getFluidState(blockposition).is(TagsFluid.WATER));

            return !holder.is(Biomes.RIVER) && !holder.is(Biomes.FROZEN_RIVER) ? random.nextInt(40) == 0 && isDeepEnoughToSpawn(worldaccess, blockposition) && flag : random.nextInt(15) == 0 && flag;
        }
    }
